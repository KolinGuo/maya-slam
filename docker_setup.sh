...
# ARG_VERSION_AUTO([v0.2.0],['Copyright 2018-2022, Kolin Guo, All rights reserved.'],[V])
# ARGBASH_GO()
...
evaluate_strictness()
{
  [[ "$2" =~ ^-(-(container-name|build-local|rm-img|bashrc-only|help|version)$|[lhV]) ]] && die "You have passed '$2' as a value of argument '$1', which makes it look like that you have omitted the actual value, since '$2' is an option accepted by this script. This is considered a fatal error."
}
  ...
  do
    ...
    case "$_key" in
      ...
      -h*)
        ...
        ;;
      -V|--version)
        printf '%s %s\n\n%s\n%s\n' "docker_setup.sh" "v0.2.0" 'Generic docker image/container setup script, adapted for maya-slam' 'Copyright 2018-2022, Kolin Guo, All rights reserved.'
        exit 0
        ;;
      -V*)
        printf '%s %s\n\n%s\n%s\n' "docker_setup.sh" "v0.2.0" 'Generic docker image/container setup script, adapted for maya-slam' 'Copyright 2018-2022, Kolin Guo, All rights reserved.'
